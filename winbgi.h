...
#define random(range) (rand() % (range))  // 0 ~ range 까지의 랜덤 수 생성

#ifndef __COLORS
#define __COLORS  // 색상 팔레트  
enum colors {
...
class WinBGI {
	...
	WinBGI() {
		bgiemu_handle_redraw = 1; // TRUE;
		bgiemu_default_mode = VGAHI; //VGAMAX;
	}
...
public:
	...
	static WinBGI& instance()
	{
		static WinBGI singletone;
		static bool bInited = false;
		if (!bInited) {
			bInited = true;
			int gd = DETECT, gm;
			singletone.initgraph(&gd, &gm, "demo");    // 그래픽 LIB 초기화하는 함수 
			//std::cout << win.getmodename(gm) << std::endl;   // 화면해상도확인
		}
		return singletone;
	}
	~WinBGI() {
	...
	void detect_mode(int* gd, int* gm);
	...
	// 음향 
	void sound(int hz);
	void sound2(int hz, int duration);
	void nosound();
	...
	void floodfill PROTO((int, int, int));  // 결계까지 채움.
	...
	void text_output(int x, int y, const char* str);
	...
	//	static void restorecrtmode PROTO((void));
