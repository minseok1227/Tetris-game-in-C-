...
//WinBGI* WinBGI::pSingletone = NULL;
#define MAX_PAGES 16
...
int WinBGI::graphresult()
{
	return gdi_error_code;
}

void WinBGI::setcolor(int c)
{
	c &= MAXCOLORS;
	color = c;
...
int WinBGI::getmaxcolor()
{
	return WHITE;
}

int WinBGI::getmaxmode()
{
	return VGAMAX;
}

char* WinBGI::getmodename(int mode)
{
...
char* WinBGI::getdrivername()
{
	return (char*)"EGAVGA";
}

void WinBGI::setlinestyle(int style, unsigned int pattern, int thickness)
{
...
void WinBGI::getlinesettings(linesettingstype* ls)
{
	*ls = line_settings;
}
...
void WinBGI::setpalette(int index, int color)
{
	color &= MAXCOLORS;
...
void WinBGI::setrgbpalette(int index, int red, int green, int blue)
{
...
void WinBGI::setallpalette(palettetype* pal)
{
...
palettetype* WinBGI::getdefaultpalette()
{
...
void WinBGI::getpalette(palettetype* pal)
{
	*pal = current_palette;
}

int WinBGI::getpalettesize()
{
	return MAXCOLORS + 1;
}

void WinBGI::setbkcolor(int color)
{
	color &= MAXCOLORS;
...
int WinBGI::getbkcolor()
{
	return bkcolor;
}

void WinBGI::setfillstyle(int style, int color)
{
...
void WinBGI::getfillsettings(fillsettingstype* fs)
{
	*fs = fill_settings;
}
...
void WinBGI::setfillpattern(char const* upattern, int color)
{
...
void WinBGI::getfillpattern(fillpatterntype fp)
{
...
void WinBGI::setusercharsize(int multx, int divx, int multy, int divy)
{
	font_mul_x = multx;
	font_div_x = divx;
	font_mul_y = multy;
...
void WinBGI::moveto(int x, int y)
{
...
void WinBGI::moverel(int dx, int dy)
{
	POINT pos;
...
void WinBGI::text_output(int x, int y, const char* str)
{
	select_font();
	if (text_color != color) {
		text_color = color;
...
void WinBGI::settextstyle(int font, int direction, int char_size)
{
	if (char_size > 10) {
		char_size = 10;
	}
...
void  WinBGI::settextjustify(int horiz, int vert)
{
...
void  WinBGI::gettextsettings(textsettingstype* ts)
{
	*ts = text_settings;
}


int  WinBGI::textheight(const char* str)
{
	SIZE ss;
	select_font();
...
int  WinBGI::textwidth(const char* str)
{
	SIZE ss;
	select_font();
	GetTextExtentPoint32(hdc[0], str, strlen(str), &ss);
...
void  WinBGI::outtext(const char* str)
{
...
void WinBGI::outtextxy(int x, int y, const char* str)
{
...
void WinBGI::setviewport(int x1, int y1, int x2, int y2, int clip)
{
...
void WinBGI::getviewsettings(viewporttype* viewport)
{
...
void WinBGI::ellipse(int x, int y, int start_angle, int end_angle,
	int rx, int ry)
{
	ac.x = x;
	ac.y = y;
...
void WinBGI::fillellipse(int x, int y, int rx, int ry)
{
	pcache.select(color + BG);
	select_fill_color();
	if (bgiemu_handle_redraw || visual_page != active_page) {
...
void  WinBGI::setactivepage(int page)
{
...
void  WinBGI::setvisualpage(int page)
{
	POINT pos;
	if (hdc[page] == NULL) {
		allocate_new_graphic_page(page);
...
void  WinBGI::setaspectratio(int ax, int ay)
{
	aspect_ratio_x = ax;
	aspect_ratio_y = ay;
}

void  WinBGI::getaspectratio(int* ax, int* ay)
{
	*ax = aspect_ratio_x;
	*ay = aspect_ratio_y;
}

void WinBGI::circle(int x, int y, int radius)
{
	pcache.select(color + BG);
...
void WinBGI::arc(int x, int y, int start_angle, int end_angle, int radius)
{
	ac.x = x;
	ac.y = y;
...
void WinBGI::getarccoords(arccoordstype* arccoords)
{
	*arccoords = ac;
}

void WinBGI::pieslice(int x, int y, int start_angle, int end_angle,
	int radius)
{
	pcache.select(color + BG);
	select_fill_color();
...
void WinBGI::sector(int x, int y, int start_angle, int end_angle,
	int rx, int ry)
{
	ac.x = x;
	ac.y = y;
...
void WinBGI::bar(int left, int top, int right, int bottom)
{
	RECT r;
...
void WinBGI::bar3d(int left, int top, int right, int bottom, int depth, int topflag)
{
	int temp;
...
void WinBGI::lineto(int x, int y)
{
	if (write_mode != COPY_PUT) {
		SetROP2(hdc[0], write_mode_cnv[write_mode]);
		SetROP2(hdc[1], write_mode_cnv[write_mode]);
...
void WinBGI::linerel(int dx, int dy)
{
	POINT pos;
	GetCurrentPositionEx(hdc[1], &pos);
...
void WinBGI::drawpoly(int n_points, int* points)
{
	if (write_mode != COPY_PUT) {
		SetROP2(hdc[0], write_mode_cnv[write_mode]);
		SetROP2(hdc[1], write_mode_cnv[write_mode]);
...
void WinBGI::line(int x0, int y0, int x1, int y1)
{
	POINT line[2];
	line[0].x = x0;
	line[0].y = y0;
...
void WinBGI::rectangle(int left, int top, int right, int bottom)
{
	POINT rect[5];
...
void WinBGI::fillpoly(int n_points, int* points)
{
	pcache.select(color + BG);
	select_fill_color();
	if (bgiemu_handle_redraw || visual_page != active_page) {
...
void WinBGI::floodfill(int x, int y, int border)
{
	select_fill_color();
	if (bgiemu_handle_redraw || visual_page != active_page) {
...
void WinBGI::delay(unsigned msec)
{
...
void WinBGI::detectgraph(int* graphdriver, int* graphmode)
{
	*graphdriver = VGA;
	*graphmode = bgiemu_default_mode;
}

int WinBGI::getgraphmode()
{
...
void WinBGI::putimage(int x, int y, void* image, int bitblt)
{
...
void WinBGI::getimage(int x1, int y1, int x2, int y2, void* image)
{
	BGIimage* bi = (BGIimage*)image;
	int* image_bits;
	bi->width = x2 - x1 + 1;
...
unsigned int WinBGI::getpixel(int x, int y)
{
	int color;
...
void WinBGI::putpixel(int x, int y, int c)
{
	c &= MAXCOLORS;
	if (bgiemu_handle_redraw || visual_page != active_page) {
	...
	WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:  // added for handling arrow keys 
		if (wParam >= 37 && wParam <= 40) {
			switch ((TCHAR)wParam) {
			case 37:
				kbd_queue.put((TCHAR)0);
				kbd_queue.put((TCHAR)75);
				break;
			case 38:
				kbd_queue.put((TCHAR)0);
				kbd_queue.put((TCHAR)72);
				break;
			case 39:
				kbd_queue.put((TCHAR)0);
				kbd_queue.put((TCHAR)77);
				break;
			case 40:
				kbd_queue.put((TCHAR)0);
				kbd_queue.put((TCHAR)80);
				break;
			}
		}
		break;
...
/* grapic device and its mode
   setting the window width and height
   modified to DETECT use 800x800 (not VGA)
*/

//static 
void WinBGI::detect_mode(int* gd, int* gm)
{
	switch (*gd) {
	case CGA:
		window_height = 200;
	...
	case DETECT:
		if (0) {
			...
			*gm = bgiemu_default_mode;
			...
			window_height = 80 * 9 + 83;
			window_width = 80 * 8 + 79;
			break;
		}
...
void WinBGI::initgraph(int* device, int* mode, char const* title /*pathtodriver*/)
{
	int index;
	...
	hWnd = CreateWindow("BGIlibrary", title,
		WS_OVERLAPPEDWINDOW,
...
void WinBGI::restorecrtmode() {}


void WinBGI::sound(int hz)
{
	Beep(hz, 100);
}

void WinBGI::sound2(int hz, int duration)
{
	Beep(hz, duration);
}
